...
```{r}
pacman::p_load(dplyr, sjPlot, stargazer, kableExtra, texreg, haven, sjmisc, ggplot2)
...
```{r}
experimento %>% ggplot(aes(x=aleatorio, y=mean))+
  geom_point()+
  ylim(1,4)+
  facet_wrap(~merit)
```
...
```{r}
experimento <- data %>% group_by(aleatorio) %>% 
